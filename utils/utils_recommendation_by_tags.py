...
def get_top_similar_games_by_tags(selected_tags, games_df, cosine_similarities, indices):
    ...
    if selected_tags:
        ...
        if not selected_games:  # 선택한 태그에 해당하는 게임이 없는 경우
            return pd.DataFrame(columns=['AppID', 'Name', 'Price', 'Developers', 'Publishers', 'Genres'])
        else:
    ...
    similarity_indices = [i[0] for i in top_similar_games]
    recommendations_df = games_df.iloc[similarity_indices][['AppID', 'Name', 'Price', 'Developers', 'Publishers', 'Tags']]
